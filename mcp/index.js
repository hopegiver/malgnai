...
server.tool(
  ...
  {
    project_id: z.string().optional().describe("특정 프로젝트로 범위 한정 (생략 시 전체)"),
  },
  async (params) => ({
...
server.tool(
  ...
  "메모리 검색 (title/content/tags LIKE 검색. 벡터 검색은 후순위). query 생략 시 최신/중요 순 목록.",
  {
    ...
    project_id: z.string().optional().describe("프로젝트 ID 필터"),
...
server.tool(
  "project_status_set",
  "프로젝트 라이프사이클 상태(대기/진행/완료/보류)를 선언한다. 라이프사이클=사람/AI의 '단계 의도'로 가동상태(파생)와 직교한다. 프로젝트가 완료됐다/보류다/다시 진행이다 라고 판단했을 때 호출. 순수 이력은 activity_log 로 충분하고 이 툴은 라이프사이클 전이 전용. 저위험(사람이 웹에서 즉시 되돌릴 수 있음)이라 승인함을 거치지 않고 직접 전이하며, 모든 전이는 감사로그에 남는다.",
  {
    project_id: z.string().describe("대상 프로젝트 ID(STATUS.md 헤더 또는 get_current_context 로 확인)"),
    status: z.enum(["pending", "active", "completed", "on_hold"]).describe("전이할 상태: pending=대기, active=진행, completed=완료, on_hold=보류"),
