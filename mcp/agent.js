    ...
    this.server.registerTool(
      'email_send',
      {
        description:
          ...
          '발신자는 malgnai@apiserver.kr로 고정되고 Reply-To는 호출한 직원 본인 주소가 된다. 본문 말미에 발송자를 밝히는 한 줄이 자동으로 붙는다. ' +
          ...
          'HTML 본문은 지정할 수 없다(text만 받고, 서버가 그 text를 이스케이프해 HTML로 변환한다 — 기본은 서식 없는 그대로지만 format:\'markdown\'을 주면 아래 서브셋 문법만 서식으로 렌더된다). 첨부파일은 지원하지 않는다. ' +
          "format:'markdown'을 주면 제목(#~###)·굵게(**)·기울임(*)·인라인코드(`)·목록(-, 1.)·링크([표시](https://…))만 서식으로 렌더된다. 표·이미지·HTML 태그는 지원하지 않고 입력한 문자 그대로 보인다. 링크는 수신자 화면에 표시텍스트와 실제 URL이 항상 함께 표시되며 https://만 링크가 된다. 표시텍스트에 <·>나 URL이 들어 있거나, 주소의 호스트 부분에 @가 있으면 링크가 되지 않고 입력한 문자 그대로 보인다(목적지 위장 방지). " +
