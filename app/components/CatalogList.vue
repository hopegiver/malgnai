<template>
  <div class="cat-list">
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
      ...
      <div class="d-flex align-items-center gap-2">
        <button
          ...
          title="GitHub malgnsoft/claude-plugins에서 즉시 다시 동기화"
        >
          <i class="bi" :class="state.syncing ? 'bi-arrow-repeat spin' : 'bi-cloud-download'"></i>
          수동 업데이트
        </button>
        <button class="btn btn-outline-secondary btn-sm d-flex align-items-center gap-1" @click="load" :disabled="state.loading">
          <i class="bi" :class="state.loading ? 'bi-arrow-repeat spin' : 'bi-arrow-clockwise'"></i>
          새로고침
        </button>
      </div>
    </div>

    <div v-if="state.syncMessage" class="alert py-2 small mb-4" :class="state.syncError ? 'alert-danger' : 'alert-success'">
      {{ state.syncMessage }}
    </div>
    ...
    <div class="cat-tabs-wrap mb-4">
      <ul class="nav cat-tabs" role="tablist">
        <li class="nav-item" role="presentation" v-for="t in TYPES" :key="t.value">
          <button class="nav-link" type="button" :class="{ active: state.activeType === t.value }" @click="state.activeType = t.value">
            <i class="bi" :class="t.icon"></i>{{ t.label }}
            <span v-if="!state.loading && !state.error" class="cat-tab-count">{{ countByType(t.value) }}</span>
          </button>
        </li>
      </ul>
    </div>
    ...
    <!-- 에러 -->
    <div v-else-if="state.error" class="alert alert-warning d-flex justify-content-between align-items-center">
      <span><i class="bi bi-exclamation-triangle me-1"></i>{{ state.errorMessage }}</span>
    ...
    <template v-else-if="sections.length">
      <div class="cat-jump-chips mb-4">
        <button
          ...
          type="button"
          class="cat-jump-chip"
          :class="{ active: state.activeSectionKey === sec.anchorId }"
    ...
    <div v-else-if="state.items.length" class="text-center py-5">
    ...
    <div v-else class="text-center py-5">
      ...
      <div class="text-faint small">malgn-agent 플러그인 동기화(관리자 수동 트리거 또는 매일 자동)가 완료되면 이곳에 표시됩니다.</div>
    </div>
  </div>
</template>
...
<script>
/**
 * CatalogList — 카탈로그 목록(타입탭+점프칩+섹션+카드그리드) 재사용 컴포넌트.
 *
 * 목적(malgnai-public 카탈로그 슬라이드오버 리팩터, 2026-08-19): app/pages/catalog/index.vue와
 * app/pages/catalog/[id].vue 양쪽에서 동일한 목록 UI를 렌더한다. vue-zero는 진짜 페이지 이동
 * (router.push)마다 대상 라우트 컴포넌트를 통째로 재마운트하므로, 두 페이지가 각자 로컬 data()로
 * items/activeType/scrollY를 들고 있으면 상세 패널을 열고 닫을 때마다 스크롤과 탭이 초기화된다.
 * 이를 막기 위해 공유 상태(state)를 <script> 최상위(모듈 스코프)에 Vue.reactive()로 한 번만
 * 만들어두고, 이 컴포넌트가 몇 번을 다시 마운트되든(= 몇 번을 다른 페이지 사이를 오가든) 항상 같은
 * 객체 참조를 data()에서 반환한다 — Vue.reactive()는 같은 target에 대해 항상 같은 proxy를 반환하므로
 * (WeakMap 캐시) data(){ return { state } }를 매 마운트 호출해도 안전하게 동일 상태를 공유한다.
 *
 * 컴포넌트 로더(vue-zero.js class L)는 components.json에 등록된 컴포넌트를 앱 부팅 시 1회만
 * fetch+import하고 캐시하므로(this.cache Map), 이 <script> 모듈 자체도 세션 내내 단 한 번만
 * 평가된다 — 즉 아래 `state`는 진짜 싱글턴이다.
 */
const state = Vue.reactive({
  ...
  error: false,
  errorMessage: '',
  loaded: false, // 최초 로드 완료 여부 — true면 재마운트 시 재요청하지 않는다.
  activeType: 'agent',
  activeSectionKey: null,
  scrollY: 0, // 카드를 클릭해 상세로 이동하기 직전의 window.scrollY. 패널을 닫거나 다른 카드로
  // 전환할 때 vue-router의 scrollBehavior(항상 {top:0})가 스크롤을 초기화하므로, 이 값으로 되돌린다.
  isAdmin: false, // "수동 업데이트" 버튼 노출 여부(POST /api/admin/catalog/sync는 requireAdmin이라
  // 관리자가 아니면 어차피 403이지만, 버튼 자체를 직원에게 보여주지 않는다).
  syncing: false,
  syncMessage: '',
  syncError: false,
})
...
export default {
  name: 'CatalogList',
  props: {
    // 상세 패널에 열려있는 항목 id. 문자열/숫자 어느 쪽으로 와도 비교되도록 isSelected()에서
    // String() 변환 후 비교(라우트 파라미터는 문자열, item.id는 API 응답에 따라 숫자일 수 있음).
  ...
  computed: {
    filteredItems() {
      return this.state.items.filter((it) => it.item_type === this.state.activeType)
    },
  ...
  watch: {
    ...
    // 이 컴포넌트 인스턴스가 재마운트 없이 유지된 채로(= [id].vue에서 다른 상세 항목으로 전환) 라우트만
    // 바뀌는 경우, vue-router의 scrollBehavior가 매번 스크롤을 top:0으로 되돌리므로 그때마다 다시 복원한다.
    '$route.fullPath'() {
      this.restoreScroll()
    },
  },
  async mounted() {
    ...
    this.state.isAdmin = me?.role === 'administrator'
    this.setupSectionObserver()
    this.restoreScroll()
  },
  beforeUnmount() {
  ...
  methods: {
    async load() {
      ...
      if (error) {
        ...
        this.state.errorMessage = error?.message || '카탈로그 목록을 불러오지 못했습니다.'
        return
      }
    ...
    // 바로 반영하고 싶을 때를 위한 트리거 — 성공/실패 요약을 배너로 보여주고 목록을 다시 불러온다.
    async syncNow() {
    ...
    isSelected(item) {
      return this.selectedId !== null && this.selectedId !== undefined && String(item.id) === String(this.selectedId)
    },
    // 카드 클릭 직전의 스크롤 위치를 저장해두고 부모에게 선택된 id만 알린다 — 실제 라우팅은 부모
    // (index.vue/[id].vue)가 담당(설계 방향 §1: emit('select', id)만 하고 직접 라우팅하지 않음).
    selectItem(item) {
    ...
    // vue-router의 scrollBehavior(vue-zero.js, 수정 불가)는 push 네비게이션마다 항상 {top:0}을
    // 반환해 스크롤을 초기화한다. 그 적용이 끝난 뒤에 우리 값으로 덮어써야 하므로 rAF를 두 번 중첩해
    // "이번 프레임의 레이아웃/스크롤 적용이 모두 끝난 다음 프레임"까지 미룬다(경험적으로 안정적인
    // 표준 우회법 — nuxt 등에서도 흔히 쓰는 패턴).
    restoreScroll() {
    ...
    setupSectionObserver() {
      ...
      this.$nextTick(() => {
        ...
        this.state.activeSectionKey = headers[0].dataset.sectionAnchor
        this._sectionObserver = new IntersectionObserver(
          (entries) => {
            ...
            if (visible.length) this.state.activeSectionKey = visible[0]
          },
...
<style>
/* 컴포넌트 스코프 스타일(vue-zero class D: scope="component")은 페이지 스코프와 달리 "현재
   활성 페이지가 아니면 disabled"되는 특성이 없어 한 번 주입되면 계속 유지된다 — 이 CatalogList는
   index.vue/[id].vue 두 페이지에서 공유되므로, 카드/탭 스타일을 어느 한쪽 페이지의 <style>에 두면
   다른 페이지에 있을 때 스타일이 빠지는 문제(malgnai-hub CLAUDE.md 상단 경고 사례)가 생긴다.
   그래서 이 스타일은 반드시 컴포넌트 파일 자체에 둔다. */
...
/* 상세 패널이 열려있는 카드 강조(슬라이드오버 리팩터 신규) — hover와 구분되도록 배경까지 함께 준다. */
.cat-card.active { border-color: var(--color-brand); box-shadow: var(--shadow-md); background-color: var(--color-canvas-soft); }
