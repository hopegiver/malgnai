# STATUS — malgnai-hub
_최종 갱신: 2026-07-28 — MCP 도구 전면 개명(엔티티_동사) + project_states 폐기(state는 즉석 계산으로 전환) 배포, https://malgnai-hub.malgnsoft.workers.dev 실배포 유지 중._
...
- **[07-28] MCP 도구 전면 개명 + `project_states` 폐기** — 11개 도구 전부 `엔티티_동사`(`project_get_context`/`work_record`/`decision_record`/`issue_record`+`issue_resolve`/`project_search_history`/`wbs_*`/`project_bootstrap`)로 개명, `update_project_state`/`project_states` 테이블 삭제(다중세션 레이스 구조적 결함) — `state`는 이제 works 최신행+wbs 롤업+open issues로 매번 즉석 계산. 배포 URL에서 상태전이 시나리오(신규→blocked→critical이슈→resolve) 전부 검증 — commit `af17690`, decision `0fb80928`.
...
- _그 이전(app/ assets 배포, WBS v1 편입, 아키텍처 확정 등)은 `decision_list`/`memory_search`로 조회._
