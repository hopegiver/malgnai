...
// (정본 decision `01m13thq2gbc0hw6tcc4yqh2sq`): latestScore/scoreHistory는 agentName→
// catalog_items→catalog_item_versions 경로로 찾은 "카탈로그 자산의 최신 버전" 스코프
// (agent-scores.js), recentLearnings는 그대로 user_id+agent_name 스코프(agent-learnings.js,
// 변경 없음) — 에이전트 코드 품질은 회사 공용 자산이지만 개인 학습 경험은 사람에게 속하기 때문.
// 3개를 독립 조회해 조합한다. 일부 조회만 실패해도(예: scores 조회만 타임아웃, 또는 agentName이
// 아직 카탈로그에 동기화되지 않아 latestScore/scoreHistory가 자연히 null/빈배열) 나머지는 정상
// 반환하고 실패한 section만 null/빈 배열로 채운다(전부 실패했을 때만 도구 전체 에러) — context.js
// §4.1과 동일 비정상 케이스 처리 원칙(§4.14).
...
function toScoreHistory(rows) {
  return rows.map((r) => ({ overallScore: r.overall_score, createdAt: r.created_at, raterType: r.rater_type, verified: !!r.verified }))
}
