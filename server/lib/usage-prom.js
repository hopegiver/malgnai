...
// ---------------------------------------------------------------------------
// PromQL 인젝션 방지(설계 §4.7) — from/to는 쿼리 파라미터로만 들어가고(이 파일에서 문자열 조립 안 함),
// D1에서 온 email만 검증+이스케이프 후 라벨 매처에 삽입한다. 요청 값(sort/order/limit)은 이 표현식
// 조립에 절대 관여하지 않는다.
// ---------------------------------------------------------------------------
...
function emailMatcher(email) {
  return `user_email="${escapeLabelValue(email)}"`
}

/** 본인 스코프 하이브리드(승인결정1, `GET /api/usage/me`) 판정 — getUsageOverviewHybrid의 신규
 *  `email` 인자를 여기 한 곳에서만 해석한다(pure — D1/네트워크 의존 없음, 단위테스트 대상).
 *  email이 없으면(관리자 전사 조회, /summary·/users 기존 호출부) 무필터 그대로.
 *  email이 있는데 PromQL 라벨매처로 안전하지 않으면(§4.7) matchers를 **비워서 무필터로 넓히지
 *  않는다** — scoped=true를 유지해 호출부(getUsageOverviewHybrid)가 이 경우 라이브 질의 자체를
 *  생략하게 한다(IDOR 방지가 "빈 매처 = 전체 조회"로 뒤집히면 안 된다). D1 캐시 스코핑은 emailLower를
 *  그대로 SQL 파라미터로 넘기므로(usage-prom-daily.js) 라벨 안전성과 무관하게 항상 유효하다. */
export function resolveEmailScope(email) {
  const scoped = typeof email === 'string' && email.length > 0
  if (!scoped) return { scoped: false, emailSafe: false, emailLower: null, matchers: [] }
  const emailSafe = isPromSafeEmail(email)
  return { scoped: true, emailSafe, emailLower: email.toLowerCase(), matchers: emailSafe ? [emailMatcher(email)] : [] }
}
...
function exprTokensByUser(duration, matchers = []) {
  return `sum by(user_email, employee_name, type) (${agg(metricSelector(METRICS.tokens, matchers), duration)})`
}
function exprSessionsByUser(duration, matchers = []) {
  return `sum by(user_email, employee_name) (${agg(metricSelector(METRICS.sessions, matchers), duration)})`
}
function exprCostByUser(duration, matchers = []) {
  return `sum by(user_email, employee_name) (${agg(metricSelector(METRICS.cost, matchers), duration)})`
}
...
function exprActiveTime(duration, matchers) {
  return `sum by(user_email) (${agg(metricSelector(METRICS.activeTime, matchers), duration)})`
}
...
function seriesEmailLower(labels) {
  const email = labels.user_email
  return typeof email === 'string' && email ? email.toLowerCase() : null
}

function ensureUserEntry(byUser, emailLower, labels) {
  let entry = byUser.get(emailLower)
  if (!entry) {
    entry = { displayEmail: labels.user_email, employeeName: labels.employee_name || null, days: new Map() }
    byUser.set(emailLower, entry)
  } else if (!entry.employeeName && labels.employee_name) {
    // 같은 user_email에 employee_name이 두 값으로 존재하면 최초 관측값을 쓴다(설계 §4.1 하단).
    entry.employeeName = labels.employee_name
  }
  return entry
}
...
function mergeTokens(byUser, unknownTypes, result, isInstant, instantDayAt) {
  for (const series of result || []) {
    const labels = series.metric || {}
    const emailLower = seriesEmailLower(labels)
    ...
    const entry = ensureUserEntry(byUser, emailLower, labels)
...
function mergeSingleField(byUser, result, field, transform, isInstant, instantDayAt) {
  for (const series of result || []) {
    const labels = series.metric || {}
    const emailLower = seriesEmailLower(labels)
    if (!emailLower) continue
...
/** grid/today 스펙 실행 + byUser/unknownTypes에 직접 병합. 스펙이 없으면(날짜창이 비어 있으면)
 ...
 *  (§19.3-4 이음매 불변식). */
async function runOverviewSpecsAndMerge(env, { grid, today, refresh, matchers, byUser, unknownTypes, profile = 'live', nowMs = Date.now() }) {
  const specs = []
  const idx = {}
  if (grid) {
  ...
  if (grid) {
    mergeTokens(byUser, unknownTypes, results[idx.tokensGrid].data, false)
    mergeSingleField(byUser, results[idx.sessionsGrid].data, 'session_count', roundNonNegative, false)
    mergeSingleField(byUser, results[idx.costGrid].data, 'cost_usd', truncateCost, false)
  }
  if (today) {
    mergeTokens(byUser, unknownTypes, results[idx.tokensToday].data, true, todayDayAt)
    mergeSingleField(byUser, results[idx.sessionsToday].data, 'session_count', roundNonNegative, true, todayDayAt)
    mergeSingleField(byUser, results[idx.costToday].data, 'cost_usd', truncateCost, true, todayDayAt)
  }
  return results
}

/** /summary, /users, /users/:id 공용 — 사용자×일 그리드를 Prometheus에서 가져와 병합한다(전 구간
 *  라이브 경로. 전사 요약 하이브리드는 아래 getUsageOverviewHybrid를 쓴다, rev.2 §0.1 C1·C9).
 *  matchers를 주면 특정 사용자로 서버측 필터링(드릴다운). matchers가 비어 있으면(전사 무필터) rev.2
 *  기준 라이브 창은 최대 3일이라(§19.2) 팬아웃 없이 그대로 질의한다(rev.1 팬아웃은 폐기, §21.1). */
export async function getUsageOverview(env, { from, to, refresh, matchers = [] } = {}) {
  ...
  const results = await runOverviewSpecsAndMerge(env, { grid, today, refresh, matchers, byUser, unknownTypes, nowMs })
  return { byUser, unknownTypes, meta: aggregateCacheMeta(results), rawResults: results }
}
...
export async function fetchCompletedDayGrid(env, { from, to, matchers = [] } = {}) {
  ...
  const byUser = new Map()
  const unknownTypes = new Set()
  if (!grid) return { byUser, unknownTypes }
  await runOverviewSpecsAndMerge(env, { grid, today: null, refresh: false, matchers, byUser, unknownTypes, profile: 'cron', nowMs })
  return { byUser, unknownTypes }
}
...
/** 전사 일별 추세(/summary) — 사용자 축을 합산해 day_at별 합계로 접는다(설계 §5.2, /users와 같은
 *  Map 결과를 축만 바꿔 접음 → KPI·표 정합이 구조적으로 보장됨. rev.2 §19.4). */
export function daySummaryRows(byUser) {
  const byDay = new Map()
...
/** D1 users × Prometheus 사용자 병합(설계 §5.4 — 양방향 불일치를 버리지 않고 흡수). */
export function mergeD1AndPromUsers(d1Users, byUser) {
  const rows = []
  const consumedEmails = new Set()
  for (const u of d1Users) {
    const emailLower = (u.email || '').toLowerCase()
    consumedEmails.add(emailLower)
    const entry = byUser.get(emailLower)
    const totals = entry ? userTotals(entry) : zeroTotals()
    rows.push({
      user_id: u.id, name: u.name, email: u.email, role: u.role, status: u.status, source: 'd1_user',
      ...totals,
      tool_calls: null, tool_errors: null, retries: null, turns: null, api_calls: null
    })
  }
  for (const [emailLower, entry] of byUser) {
    if (consumedEmails.has(emailLower)) continue
    const totals = userTotals(entry)
    rows.push({
      // m-6 수정(리뷰 2026-09-03): entry.displayEmail은 캐시 유래 행이 있으면 소문자(§17.3 저장
      // 규칙), 라이브에만 있으면 Prometheus 라벨 원본 대소문자다 — 어느 날짜 세그먼트가 먼저
      // 병합됐는지에 따라 같은 사용자의 표시 이메일이 요청마다 대소문자를 오갈 수 있었다. 표시용은
      // 항상 emailLower(맵 키, 항상 소문자)로 통일한다.
      user_id: null, name: entry.employeeName || null, email: emailLower,
      role: null, status: 'unregistered', source: 'prometheus_only',
      ...totals,
      tool_calls: null, tool_errors: null, retries: null, turns: null, api_calls: null
    })
  }
...
/** GET /api/admin/usage/users/:id 전용 — email이 PromQL 라벨매처로 안전하지 않거나 Prometheus에
 *  한 건도 없으면 notInMetrics=true, 200(빈 데이터)로 처리한다(§5.3 — 404가 아니다). rev.2에서도
 *  변경 없음(전 구간 라이브 유지, §0.1 C9). */
export async function getUserDrilldown(env, { from, to, email, refresh }) {
  if (!isPromSafeEmail(email)) {
    return { notInMetrics: true, rows: [], totals: { ...zeroTotals(), active_time_seconds: 0 }, byModel: [], unknownTypes: new Set(), meta: aggregateCacheMeta([]) }
  }

  const matchers = [emailMatcher(email)]
  ...
  const entry = overview.byUser.get(emailLower)
...
// ---------------------------------------------------------------------------
// rev.2 하이브리드 합성(설계 §19.4) — /summary·/users 공용. "완결일=D1 롤업 캐시 + 오늘=라이브"를
// 이 함수 안에서만 합성한다. /summary·/users는 반환된 byUser Map을 축만 바꿔 접는다
// (daySummaryRows/mergeD1AndPromUsers, 기존 함수 그대로 재사용).
// ---------------------------------------------------------------------------

/** 캐시 구간 원본 행(day_at,user_email 그레인)을 라이브와 동일한 Map<emailLower,Map<dayAt,values>>
 *  구조에 병합한다 — 이래야 §19.4가 요구하는 "캐시 행을 Map에 넣고 라이브를 같은 Map에 병합"이
 *  성립하고, daySummaryRows/userTotals/mergeD1AndPromUsers를 캐시·라이브 어느 쪽 유래든 그대로
 *  재사용할 수 있다(집계 로직 이중 구현 방지). */
function mergeCacheRows(byUser, rows) {
  for (const r of rows) {
    const emailLower = (r.user_email || '').toLowerCase()
    if (!emailLower) continue
    const entry = ensureUserEntry(byUser, emailLower, { user_email: r.user_email, employee_name: r.employee_name })
...
/** 캐시 Map과 라이브 Map을 결합한다. W2(§19.3)에 의해 같은 (email,day)가 두 Map에 동시에 존재할 수
 *  없어야 한다 — 있으면 설계상 불가능해야 하는 이중계상 버그이므로 캐시 값을 보존하고 시끄럽게
 *  로그한다(§19.4 "키가 이미 있으면 그것은 버그다"). */
function combineByUserMaps(cacheByUser, liveByUser) {
  const combined = new Map()
  for (const [emailLower, entry] of cacheByUser) {
    combined.set(emailLower, { displayEmail: entry.displayEmail, employeeName: entry.employeeName, days: new Map(entry.days) })
  }
  for (const [emailLower, liveEntry] of liveByUser) {
    let entry = combined.get(emailLower)
    if (!entry) {
      entry = { displayEmail: liveEntry.displayEmail, employeeName: liveEntry.employeeName, days: new Map() }
      combined.set(emailLower, entry)
    } else {
      // n-1 수정(리뷰 2026-09-03): 이전에는 else-if 체인이라 employeeName 보정이 일어나면
      // displayEmail 보정은 절대 도달하지 못했다(캐시 행은 displayEmail이 항상 채워져 있어 지금까지
      // 무해했지만, 두 보정은 서로 독립적인 사실이라 체인으로 묶일 이유가 없다). 독립 if로 분리.
      if (!entry.employeeName && liveEntry.employeeName) entry.employeeName = liveEntry.employeeName
      if (!entry.displayEmail && liveEntry.displayEmail) entry.displayEmail = liveEntry.displayEmail
    }
    for (const [dayAt, values] of liveEntry.days) {
      if (entry.days.has(dayAt)) {
        console.error('[usage-prom] hybrid merge collision (cache+live both have this day) — keeping cache value', emailLower, dayAt)
        continue
      }
 ...
 *
 *  email(승인결정1, `GET /api/usage/me`가 신규로 넘긴다) — 생략하면 기존 /summary·/users와 완전히
 *  동일하게 무필터(전사). 값을 주면 resolveEmailScope()가 캐시(D1 SQL 파라미터)·라이브(PromQL 라벨
 *  매처) 양쪽을 그 사용자 하나로 좁힌다 — 호출부가 반환된 byUser Map에서 다른 사용자 키를 절대
 *  받을 수 없다(스코프가 조회 단계에서부터 걸린다, 응답 필터링이 아니다). */
export async function getUsageOverviewHybrid(env, { from, to, refresh, email } = {}) {
  const nowMs = Date.now() // m-1 — 이 요청 전체에서 "지금"을 한 번만 고정해 아래로 그대로 넘긴다
  const scope = resolveEmailScope(email)
  const todayStr = todayUTCString(nowMs)
  ...
  const snapshot = await readHybridSnapshot(env.DB, from, cacheTo, AGG_MODE, NORM_VERSION, scope.emailLower)

  const cacheByUser = new Map()
  mergeCacheRows(cacheByUser, snapshot.validRows)
  ...
  let liveByUser = new Map()
  let liveUnknown = new Set()
  let liveResults = []
  let liveErr = null

  // 승인결정1 — scoped인데 emailSafe가 아니면(§4.7 라벨 인젝션 위험 문자) 라이브 질의 자체를 생략한다.
  // matchers를 빈 배열로 두고 그대로 질의하면 "안전하지 않은 email"이 "무필터(전사)"로 뒤집혀 스코프가
  // 깨진다 — 이 경우 오늘 구간은 gap_days로 정직하게 빠지고, 캐시 구간(이미 SQL로 스코프됨)만 응답한다.
  if (isToday && (!scope.scoped || scope.emailSafe)) {
    ...
    try {
      ...
      // (cacheByUser, snapshot 등)까지 예산 초과 시 통째로 버려지고 응답이 503이 된다 — 상류가
      ...
      liveResults = await withRouteBudget(runOverviewSpecsAndMerge(env, { grid, today: todayW, refresh, matchers: scope.matchers, byUser: liveByUser, unknownTypes: liveUnknown, nowMs }))
    } catch (err) {
  ...
  const effectiveLiveByUser = liveUnavailable ? new Map() : liveByUser
  const effectiveLiveUnknown = liveUnavailable ? new Set() : liveUnknown

  const combined = combineByUserMaps(cacheByUser, effectiveLiveByUser)
