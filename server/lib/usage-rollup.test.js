...
describe('computeTargetDays — W1 불변식(오늘은 절대 롤업 대상이 아니다, 설계 §19.3-1)', () => {
  ...
  it('trailing rescan 구간(어제부터 최근 N일)은 커버리지 유무와 무관하게 항상 대상에 포함된다', async () => {
    ...
    // 이미 유효 마커가 있어도(agg_mode/norm_version 일치) trailing 구간은 무조건 재적재 대상이다.
    selectCoverageRange.mockResolvedValueOnce([{ day_at: yesterday, agg_mode: 'increase', norm_version: 'v1', user_rows: 5 }])
    const targets = await computeTargetDays(fakeDb)
  ...
  it('유효 마커가 있는(agg_mode/norm_version 일치) 과거 날짜는 결손이 아니므로 대상에서 빠진다', async () => {
    const today = todayUTCString()
    const oldDay = addDaysUTC(today, -(ROLLUP_TRAILING_RESCAN_DAYS + 2))
    selectCoverageRange.mockResolvedValueOnce([{ day_at: oldDay, agg_mode: 'increase', norm_version: 'v1', user_rows: 3 }])
    const targets = await computeTargetDays(fakeDb)
  ...
  it('agg_mode/norm_version이 현재 값과 다른 과거 마커는 결손으로 간주해 재적재 대상에 포함된다', async () => {
    ...
    selectCoverageRange.mockResolvedValueOnce([{ day_at: staleDay, agg_mode: 'old-mode', norm_version: 'v0', user_rows: 3 }])
    const targets = await computeTargetDays(fakeDb)
